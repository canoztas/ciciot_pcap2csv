...
Key points:
- Outputs exactly 39 columns matching the typical CICIoT2023 example CSV header (no 'ts').
...
import argparse, sys, os, math, socket, struct
...
def compute_window_features(window: List[dict], include_igmp: bool) -> dict:
    ...
    directions = np.array([p["dir"] for p in window], dtype=int)
    ...
    has_igmp = include_igmp and any((p["is_ip"] and p["l3_proto"] == 2) for p in window)  # IPv4 proto=2
    ...
    avg_len = float(np.mean(sizes)) if sizes.size else 0.0
    std_len = float(np.std(sizes)) if sizes.size else 0.0

    # l3 majority proto (for "Protocol Type") and TTL mean ("Time_To_Live")
    ...
    row = {
        ...
        "AVG": avg_len,
        "Std": std_len,
        "Tot size": avg_len,   # the original dataset uses both Tot sum (bytes) and Tot size (paper ambiguous). We'll keep AVG here.
        "IAT": iat_metric,
        "Number": int(len(window)),
        "Variance": float(np.var(sizes)) if sizes.size else 0.0,
    }
    return row
...
def _run_with_dpkt(pcaps: List[str], out_csv: str, window: int, label: Optional[str]):
    ...
    def eth_total_len(eth):
        try:
            return len(bytes(eth))
        except Exception:
            if is_ipv4(eth): return 14 + int(eth.data.len)
            if is_ipv6(eth): return 14 + 40 + int(eth.data.plen)
            return 60
        ...
        for ts, buf in reader:
            ...
            if is_arp(eth):
                ...
                if isinstance(l4, dpkt.tcp.TCP):
                    ...
                    info["hdr_len"]= ip.hl*4 + 8; flags={"fin":0,"syn":0,"rst":0,"psh":0,"ack":0,"ece":0,"cwr":0}
                elif isinstance(l4, dpkt.icmp.ICMP):
                    info["l4"]="ICMP"; info["src"],info["dst"]=src,dst; info["hdr_len"]= ip.hl*4 + 8
                    flags={"fin":0,"syn":0,"rst":0,"psh":0,"ack":0,"ece":0,"cwr":0}
                else:
                    info["src"],info["dst"]=src,dst; info["hdr_len"]=ip.hl*4; flags={"fin":0,"syn":0,"rst":0,"psh":0,"ack":0,"ece":0,"cwr":0}
            elif is_ipv6(eth):
                ip6 = eth.data
                ...
                l4, proto = get_transport(ip6)
                if isinstance(l4, dpkt.tcp.TCP):
                    info["l4"]="TCP"; info["sport"],info["dport"]=get_ports(l4); info["src"],info["dst"]=src,dst
                    info["hdr_len"]= 40 + l4.off*4; flags = tcp_flags(l4)
                elif isinstance(l4, dpkt.udp.UDP):
                    info["l4"]="UDP"; info["sport"],info["dport"]=get_ports(l4); info["src"],info["dst"]=src,dst
                    info["hdr_len"]= 40 + 8; flags={"fin":0,"syn":0,"rst":0,"psh":0,"ack":0,"ece":0,"cwr":0}
                elif isinstance(l4, dpkt.icmp6.ICMP6):
                    info["l4"]="ICMP"; info["src"],info["dst"]=src,dst; info["hdr_len"]= 40 + 8
                    flags={"fin":0,"syn":0,"rst":0,"psh":0,"ack":0,"ece":0,"cwr":0}
                else:
                    info["src"],info["dst"]=src,dst; info["hdr_len"]=40; flags={"fin":0,"syn":0,"rst":0,"psh":0,"ack":0,"ece":0,"cwr":0}
            else:
                info["is_llc"] = True
                flags={"fin":0,"syn":0,"rst":0,"psh":0,"ack":0,"ece":0,"cwr":0}
            ...
            min_side = (min(info["src"], info["dst"]), min(sport if sport is not None else -1, dport if dport is not None else -1))
...
def _parse_tcp(pkt: bytes, off: int):
    ...
    return {"sport":sport, "dport":dport, "hdr_len": data_off*4,
            "flags":{"fin":fin,"syn":syn,"rst":rst,"psh":psh,"ack":ackf,"ece":ece,"cwr":cwr}}
...
def _run_pure_pcap(pcaps: List[str], out_csv: str, window: int, label: Optional[str]):
    ...
    for p in pcaps:
        for ts, buf in _read_pcap(p):
            ...
            min_side = (min(src_ip, dst_ip), min(sport if sport is not None else -1, dport if dport is not None else -1))
    ...
    if df.empty:
        ...
        for c in ordered_cols:
            if c not in df.columns:
                df[c] = 0 if c not in ("HTTP","HTTPS","DNS","Telnet","SMTP","SSH","IRC","TCP","UDP","DHCP","ARP","ICMP","IGMP","IPv","LLC") else 0
        # drop extras and reorder
        df = df.loc[:, ordered_cols]

    if label is not None:
        df.insert(len(ordered_cols), "Label", label)

    df.to_csv(out_csv, index=False)

def main():
    ap = argparse.ArgumentParser(description="PCAP -> CICIoT-style CSV (no timestamp column)")
